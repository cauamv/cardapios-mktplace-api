...
public class CardapioServiceImpl implements CardapioService{
	...
	public Cardapio inserir(NovoCardapio novoCardapio) {
		...
		cardapio.setRestaurante(novoCardapio.getRestaurante());
	...
	private void atualizarPrecosDas(List<OpcaoDoCardapio> opcoesDoCardapio) {
		for (OpcaoDoCardapio opcaoDoCardapio : opcoesDoCardapio) {
			if (opcaoDoCardapio.getOpcao().isEmPromocao()) {
				...
				BigDecimal precentualDeDesconto = opcaoDoCardapio.getOpcao().getPercentualDeDesconto();
				
				BigDecimal valorDecontado = opcaoDoCardapio.getPreco().multiply(precentualDeDesconto).divide(divisor);
				
				BigDecimal preco = opcaoDoCardapio.getPreco().subtract(valorDecontado).setScale(2, RoundingMode.CEILING);
